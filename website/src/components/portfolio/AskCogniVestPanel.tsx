...
import { streamClientChat } from "../../utils/cognivest-api";
...
// ── Bezier typing animation — mirrors AdvisorChatPanel ────────────────────────
...
// ── Component ─────────────────────────────────────────────────────────────────
export function AskCogniVestPanel({ open, onClose, clientId }: Props) {
  ...
  const handleSend = useCallback((text?: string) => {
    ...
    streamClientChat(
      ...
      () => {
        ...
        if (!schedulerActive.current) scheduleRef.current();
      },
      (err) => {
        setIsLoading(false);
        setIsStreaming(false);
  ...
  }, [clientId, enqueueChars, inputValue, isLoading, isStreaming, messages]);
          ...
          <div className="flex-shrink-0 px-5 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-foreground/90 tracking-wide">Ask CogniVest</span>
                <span className="ml-1 px-2 py-0.5 text-[10px] font-medium rounded-full bg-violet-500/15 text-violet-400 border border-violet-500/20">
                  AI
                </span>
              </div>
            ...
            {messages.length === 0 && (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 opacity-60">
                ...
                <p className="text-sm text-muted-foreground max-w-[220px] leading-relaxed">
                  Ask me anything about finance, investing, or how markets work.
                </p>
                <p className="text-[11px] text-muted-foreground/50 max-w-[230px]">
                  For personalised advice, speak to your CogniVest advisor.
                </p>
              </div>
            )}
          ...
          <div className="flex-shrink-0 px-4 py-4 border-t border-border/30">
            ...
            <div className="flex items-center gap-2 bg-foreground/5 rounded-xl px-4 py-2.5 border border-border focus-within:border-foreground/20 transition-colors">
              <input
                ...
                placeholder="Ask about investing or markets…"
            ...
            <p className="text-[10px] text-muted-foreground/40 text-center mt-2">
              Educational only · Not personalised advice · Powered by CogniVest AI
            </p>
          </div>
        </motion.div>
      )}
